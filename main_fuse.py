...
def train(model, epoch):
    ...
    lambd0 = torch.nn.Parameter(torch.rand(1)).cuda()
    for i, (vid, feature, label) in enumerate(train_video_loader):
        ...
        loss_RL1, output1, rewards1, locations1 = agent.rollout(feature[0].cuda(), label)
         
        loss_RL2, output2, rewards2, locations2 = agent.rollout(feature[1].cuda(), label)
        
        output = output1 + output2

        loss_CE = criterion(output, label) 
        
        loss = loss_RL1 + lambd0 * loss_RL2 + lambd * loss_CE
    ...
    tf_writer.add_scalar('loss_RL1/train', loss_RL1, epoch)
    tf_writer.add_scalar('loss_RL2/train', loss_RL2, epoch)
